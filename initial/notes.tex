    ...
    \item $\omega_{s1}$ comes from the action of the central star on the
        ...
        \begin{align}
            \omega_{s1} &\sim \frac{GM_* J_2 m_1 R_1^2/a^3}{k_p m_1 R_1^2s},\\
                &\sim \frac{k_{qp}}{k_p}\frac{M_*}{m_1} \p*{\frac{R_1}{a}}^3.
        \end{align}
...
Note that for Jupiter, $k_p \sim 0.27$ while $J_2 \sim \scinot{1.4}{-2}$ giving
$k_{qp} \sim 0.0467$ since $k_{qp} \frac{\Omega}{\sqrt{GM_p/R_p^3}} = J_2$.
Consider again the precession frequencies
\begin{align}
    \omega_{s1} &= \frac{3k_{qp}}{2k_p}\frac{M_*}{m_1}
            \p*{\frac{R_1}{a_1}}^3s,\\
        &\approx 10\;\mathrm{yr^{-1}}
            \p*{\frac{M_*/m_1}{300}}
            \p*{\frac{5.5 k_{qp}}{k_p}}\p*{\frac{R_1}{2R_J}}^3
                \p*{\frac{a_1}{0.05\;\mathrm{AU}}}^{-3}
                \p*{\frac{s}{2\pi / 10\;\mathrm{hr}}},\\
    \omega_{1p} &= \frac{3m_p}{4m_1} \frac{a_1}{a_p\p*{1 - e_p^2}} \Omega_1,\\
        &\approx 0.0625\;\mathrm{yr^{-1}}
            \frac{m_p/m_1}{0.003}
            \p*{\frac{a_1/a_p}{0.05}}
            \frac{\Omega_1}{2\pi / 4\;\mathrm{days}}
            \frac{1}{1 - e_p^2}.
\end{align}
Let's note that the effective initial $\eta = \frac{\omega_{1p}\cos
I}{\omega_{s1}} \sim 0.006$. Furthermore, the smallest $\eta$ we can get at
small final obliquities is just when $s = \Omega_1$, which is roughly a factor
of $10$ increase so $\eta_f \sim 0.06$. Recall that $\eta = \frac{s_c}{s}
\approx \frac{s_c}{\Omega_1}0.1$ since $\Omega_1/s \approx 0.1$, then $s_c
\approx 0.06$ as well.

Let's also evaluate
\begin{align}
    \rd{\theta}{t} &\approx -\frac{1}{t_a}\frac{L}{2S}\sin \theta\p*{1 -
        \frac{s}{2\Omega_1}\cos \theta},\\
        &\approx 14\;\mathrm{Myr}\p*{\frac{P}{3\;\mathrm{d}}}^{16/3}
            \p*{\frac{Q_p'}{10^7}}
            \p*{\frac{M_*}{10^3M_p}}
            \p*{\frac{a/r}{100}}^2
            \p*{\frac{s}{2\pi / 10\;\mathrm{h}}}^{-1}
            \p*{\frac{k_p}{0.25}}^{-1}
            \sin \theta\p*{1 - \frac{s}{2\Omega_1}\cos \theta}.
\end{align}
Note that typically, when trapped in CS2 and $\eta \ll 1$, $\sin \theta \approx
1$, while when at CS1, $\sin \theta \approx \sin I$. For reference, $\sin
5^\circ \approx 0.087$.

Again though, we see that if $s > \frac{2\Omega_1}{\cos\theta}$, then
$\rd{\theta}{t}$ drives towards CS2.
