...
\frac{\omega_{s1}}{\omega_{1p}\cos I}$ and $\epsilon\frac{\Omega_1}{s} =
...
\begin{align}
    \rd{\hat{s}}{\tau}
        &= \frac{s}{s_c}\p*{\hat{s} \cdot \hat{l}_1}\p*{\hat{s} \times \hat{l}_1}
            - \hat{s} \times \hat{l}_p
            + \frac{\epsilon \Omega_1}{s}
                \p*{1 - \frac{s}{2\Omega_1}\p*{\hat{l}_1 \cdot \hat{s}}}
                    \hat{s} \times \p*{\hat{l}_1 \times \hat{s}},\\
    \rd{s}{\tau}
        &= \epsilon \Omega_1 \p*{\hat{s} \cdot \hat{l}_1 - \frac{s}{2\Omega_1}\p*{1
...
\begin{align}
    ...
    \pd{\mu}{\tau} &= -\sin I \sin \phi +
        \epsilon \frac{\Omega_1}{s}\p*{1 - \mu^2}
            \p*{1 - \frac{s}{2\Omega_1}\mu},\\
    \rd{s}{\tau}
        &= \epsilon \Omega_1 \p*{\mu - \frac{s}{2\Omega_1}\p*{1 + \mu^2}}.
\end{align}
...
\begin{align}
    \pd{\mu}{(\epsilon\tau)} &\approx \frac{\Omega_1}{s}\p*{1 - \mu^2}
            \p*{1 - \frac{s}{2\Omega_1}\mu},\nonumber\\
        &\approx \p*{1 - \mu^2}
            \p*{\frac{\Omega_1}{s} - \frac{1}{2}\mu},\\
...
s(0)e^{-\frac{\epsilon}{2}\tau}$, so the critical synchronization
timescale is $\tau_{sync} \sim \frac{2}{\epsilon}$.
...
the CS\@. This tells us that at displacements of $\delta_0 \gtrsim
e^{-2}$ is where states will be able to reach Cassini States before
synchronization happens. This reinforces our expectation that synchronization is
probably no faster than reaching the Cassini state, but that a significant
fraction of initial conditions below the separatrix should be able to reach a
Cassini State before tides synchronize.
