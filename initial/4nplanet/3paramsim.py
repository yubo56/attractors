...
def plot_outcomes(I2=0, g2=0, tf=1000, eps_tide=1e-2,
                  ...
                  num_pts=5000):
    q_vals = np.arccos(np.random.uniform(-0.95, 0.95, num_pts))
    ...
    if not os.path.exists(pkl_fn2):
        ...
        # with lzma.open('foo.pkl', 'wb') as f:
        #     pickle.dump(rets[165], f)
        #     return
    ...
    #     np.array(q_fs) > 30,
    #     np.logical_and(np.cos(np.radians(q_is)) > 0.5, np.array(phi_is) < 50)))[0]
    # print(sample_idxs)
    # print(np.array(q_fs)[sample_idxs])
    # return
    if plt is not None:
