...
def plot_areas(I=np.radians(5), filename='1_areas'):
    ...
    ax1.plot(eta, A1w / (4 * np.pi), 'g', label=r'$\mathcal{A}_{\rm I}$', lw=LW)
    ax1.plot(eta, A2ys / (4 * np.pi), 'b:', lw=LW/2, alpha=0.7)
    ax1.plot(eta, A2w / (4 * np.pi), 'b', label=r'$\mathcal{A}_{\rm II}$', lw=LW)
    ax1.plot(eta, A3ys / (4 * np.pi), 'r:', lw=LW/2, alpha=0.7)
    ax1.plot(eta, A3w / (4 * np.pi), 'r', label=r'$\mathcal{A}_{\rm III}$', lw=LW)
