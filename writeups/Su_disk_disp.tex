...
$\uv{s}$ and $\uv{l}_{\rm d}$ being on the same side of $\uv{l}$.
The CS obliquity satisfie
\begin{equation}
    \sin \theta \cos \theta - \eta \sin\p{\theta - I} = 0.
\end{equation}
...
In this appendix, we study the evolution of the planetary obliquity $\theta$ when
...
The above calculation is performed for initial condition $\theta_{\rm i}$, but,
as discussed earlier in the text, the initial spin orientation is more
appropriately described by $\theta_{\rm sd, i}$. The correct way to predict
final obliquities for such initial conditions using Eq.~\eqref{eq:nonad_q_f} is
somewhat indirect but yields good agreement with numerical results. We develop
the procedure for two cases: (i) for $\theta_{\rm sd, i} = 0$, then (ii) for
nonzero $\theta_{\rm sd, i}$. To simplify the discussion below, we consider the
limit where $\eta_{\rm i} \to \infty$, and CS2 and $\uv{l}_d$ coincide. The
nonadiabatic evolution is then described by:
\begin{itemize}
    \item When $\theta_{\rm sd, i} = 0$, the initial condition is just CS2, so
        Eq.~\eqref{eq:nonad_q_f} can be directly applied setting $\theta_{\rm i}
        = I$. This yields good agreement with numerical results (see
        Figs.~\ref{fig:nonad_3_scan} and~\ref{fig:nonad_3_scan_20}). We denote
        this final obliquity
        \begin{equation}
            \theta_{\rm f} = \sqrt{\frac{2\pi}{\epsilon}}\frac{\sin (2I)}{2}.
                \label{eq:theta_2f}
        \end{equation}
        The azimuthal angle of the spin vector does not change and remains $\phi
        = \pi$. Figures~\ref{fig:nonad_3_scan} and~\ref{fig:nonad_3_scan_20}
        show the final obliquity as function of $\epsilon$ for $\theta_{\rm
        sd,i} = 0$ and $I = 5$, $20^\circ$. We see that the agreement between
        the numerical results and Eq.~\eqref{eq:nonad_q_f} is excellent,
        particularly for the $I = 5^\circ$ case. The agreement when $I =
        20^\circ$ is slightly poorer as $\theta_{\rm i}$ is larger and hence
        less negligible in Eq.~\eqref{eq:nonad_q_f}. For $\epsilon \ll
        \epsilon_{\rm c}$, we find $\theta_f \simeq 90^\circ$, in agreement with
        the result of adiabatic evolution (see Fig.~\ref{fig:ad_ensemble}).

    \item When $\theta_{\rm sd, i}$ is permitted to be nonzero, the set of
        possible initial conditions forms a ring of angular width $\theta_{\rm
        sd, i}$ about CS2. We find that this set of initial conditions
        approximately evolves under nonadiabatic evolution to a ring of angular
        width $\theta_{\rm sd, i}$ about the $\theta_{\rm sd, i} = 0$ prediction
        given by Eq.~\eqref{eq:theta_2f}. The range of possible final
        obliquities is the range of $\theta$ spanned by this ring of possible
        outcomes. This can be solved in closed form, yielding generalization of
        Eq.~\eqref{eq:theta_2f}
        \begin{equation}
            \abs{\sqrt{\frac{2\pi}{\epsilon}}\frac{\sin \p{2I}}{2}
                - \theta_{\rm sd, i}}
            \leq \theta_{\rm f}
            \leq \sqrt{\frac{2\pi}{\epsilon}}\frac{\sin\p{2I}}{2}
                + \theta_{\rm sd, i} .\label{eq:nonad_q_f_dist}
        \end{equation}
        The absolute value in the lower bound is a result of the ring of final
        conditions crossing the North pole. Fig.~\ref{fig:nonad_3_ensemble}
        shows the numerical result of $\theta_f$ vs $\theta_{\rm sd,i}$ for $I =
        5^\circ$ and $\epsilon = 0.3$. We see that Eq.~\eqref{eq:nonad_q_f_dist}
        provides good lower and upper bounds of the final obliquity for
        $\theta_{\rm sd,i} \lesssim 45^\circ$.
\end{itemize}
...
\begin{figure}
    ...
    \caption{$\theta_{\rm  f}$ vs $\theta_{\rm sd, i}$ for $I=5^\circ$
    and $\epsilon = 0.3$ (firmly in the nonadiabatic regime). The blue
    dots represent numerical results, and the two red lines show the
    analytical lower and upper limits given by
...
determine the stability of each CS and the local libration frequencies or growth
rates of the CSs.
...
\begin{itemize}
    \item CS2: $\phi_2 = \pi$, $\theta_2 \simeq I - \sin I \cos I / \eta$.
    \item CS3: $\phi_3 = 0$, $\theta_3 \simeq \pi - I + \sin I \cos I / \eta$.
\end{itemize}
