...
determined by $\dot\Phi = 0$, occuring when $\eta_0 = 1/\cos I$. We then find,
for $\tau \gg \tau_0$,
\begin{equation}
    S(\tau)e^{-i\Phi(\tau)} - S(\tau_{\rm i})
...
\begin{equation}
    S(\tau)e^{-i\Phi(\tau)} - S(\tau_{\rm i})
        \simeq -i^{3/2}\tan I \cos \theta_{\rm i} e^{-i\Phi(\tau_0)}
            \p{\frac{2\pi}{\epsilon}}^{1/2}.
\end{equation}
If the initial obliquity is much smaller than the final obliquity ($\theta_{\rm
i} = \abs{S(\tau_{\rm i})} \ll \abs{S(\tau_{\rm f})}$), then we obtain
\begin{equation}
    \theta_{\rm f}\simeq \sqrt{\frac{2\pi}{\epsilon}}\tan I\cos \theta_{\rm i}.
        \label{eq:nonad_q_f}
\end{equation}
...
$\theta_{\rm i} = \theta_2 \approx I$. The set of possible initial conditions
for nonzero $\theta_{\rm sd, i}$ is a ring of angular width $\theta_{\rm sd, i}$
about CS2. We find this ring evolves to a new ring of angular width $\theta_{\rm
sd, i}$ about the $\theta_{\rm sd, i} = 0$ prediction, so the range of final
obliquities is the $\theta$ range spanned by this final ring\footnote{The
interpretation for this procedure is that the process generating the final
obliquity predicted by Eq.~\eqref{eq:nonad_q_f} acts only on the \emph{center}
of libration of the trajectory when $\eta \gg 1$ (CS2). This is not apparent
from our derivation but reproduces the results of the numerical simulation.}.
This procedure yields prediction
\begin{equation}
    \abs{\p{\sqrt{\frac{2\pi}{\epsilon}}\tan I\cos I - \theta_{\rm sd, i}}}
    \leq \theta_{\rm f}
    \leq
    \sqrt{\frac{2\pi}{\epsilon}}\tan I\cos I + \theta_{\rm sd, i}
