...
and ``non-adiabatic'' evolution. \textcolor{Corr}{Our chief results include
that: (i) careful accounting of phase space area across separatrix
crossings is able to explain the rich dynamics of adiabatic evolution, and (ii)
the concept of ``partial adiabatic resonance advection'' fully explains dynamics
in the non-adiabatic limit.} We calibrate our analytical results with numerical
calculations.
...
\citep{henrard1987,ward2004I}. \textcolor{Corr}{As given in
\citet{ward2004I}, if we define}
\begin{subequations}
    \begin{align}
        z_0 &= \eta\cos I, &
...
\begin{figure}
    ...
    $\mathcal{A}_{\rm i}(\eta) / 4\pi$ as given by Eqs.~\eqref{se:area_ward} for
    $I = 5^\circ$. The colored dotted lines correspond to small $\eta$
    approximations used in Appendix~\ref{s:ad_approx}. The colored dashed lines
    for $\eta > \eta_{\rm c}$ are the effective values of $\mathcal{A}_{\rm II},
    \mathcal{A}_{\rm III}$ for $\eta > \eta_{\rm c}$, denoting the points that
    would flow into either area under adiabatic decrease of $\eta$ from $\eta >
    \eta_{\rm c}$ (see the text). The vertical black dashed lines correspond to
    $\eta = \eta_{\rm c}$ [Eq.\eqref{eq:etac}] and the values of $\eta$ for
    which $\mathcal{A}_{\rm II}$ is maximized ($\eta_{\rm \max, II}$) and for
    which $\mathcal{A}_{\rm III}$ is minimized ($\eta_{\rm \min, III}$,
...
\begin{figure}
    ...
    evolving from initial $\eta_{\rm i} \gg 1$ to $\eta_{\rm f} \ll 1$, for $I =
    5^\circ$. The blue dots are results of numerical calculations
    (Section~\ref{ss:ad_ensemble}), and the colored tracks are semi-analytical
    results (Section~\ref{ss:zone_transitions}). Bottom: The probabilities of
    different outcomes. Where a particular $\theta_{\rm sd, i}$ corresponds to
    multiple tracks, the system evolves probabilistically. The track that a
