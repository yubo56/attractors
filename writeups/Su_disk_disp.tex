...
\affiliation{Cornell Center for Astrophysics and Planetary Science, Department
of Astronomy, Cornell University, Ithaca, NY 14853, USA}
\affiliation{Tsung-Dao Lee Institute, Shanghai Jiao Tong University, Shanghai, 200240,
China}
...
adiabatic. \textcolor{Corr}{Intuitively, this requires the disk evolution time
$t_{\rm d}$ [Eq.~\eqref{eq:dmd_dt}] be larger than the spin precession period,
$2\pi / \alpha$, i.e.\ $\epsilon = 1 / (\alpha t_{\rm d}) \lesssim 1/ (2\pi)$.}

Strictly speaking, \textcolor{Corr}{adiabaticity} requires $t_{\rm d}$ be much
larger than all timescales of the dynamical system governed by the Hamiltonian
...
\textcolor{Corr}{timescale for the} motion of the equilibria
\textcolor{Corr}{due to changing $\eta$}. If this criterion is satisfied, then
the system will evolve adiabatically for most of its
evolution save one or two separatrix crossings.
...
\end{equation}
is the libration frequency about CS2 \textcolor{Corr}{for a given $\eta$}
...
Since $\theta_2 \sim I$ for $\eta \gtrsim 1$ (Fig.~\ref{fig:cs_locs}), this
...
\textcolor{Corr}{Eq.~\eqref{eq:ad_base} is most constraining at $\eta \sim 1$,
i.e.\ it is satisfied for all $\eta$ if it is satisfied near $\eta \sim 1$,
$\abs{\rdil{\theta_2}{\tau}} \sim \epsilon$}. Thus, weak adiabiticity requires
\textcolor{Corr}{
\begin{equation}
    \epsilon \lesssim \epsilon_{\rm c}
        \equiv \p{\frac{\omega_{\rm lib}}{2\pi}}_{\eta = 1}
            % =\frac{1}{2\pi}\sqrt{\sin I \sin \theta_{\rm 2c}
            %     \p{1 + \sin I \csc^3 \theta_{\rm 2c}}}
            \sim 0.0433,\label{eq:ad_constr}
\end{equation}
where we have used $\sin \theta_{\rm 2} \sim 1/2$ when $\eta =
1$ (for $I = 5^\circ$ and $\eta = 1$, $\theta_{\rm 2} \approx 31^\circ$)}. Since
our criterion is only a weak condition for adiabaticity, we use $\epsilon = 3
\times 10^{-4}$ in our ``adiabatic'' calculations below. We explore the
...
\begin{figure}
    \centering
    \includegraphics[width=0.7\columnwidth]{plots_diskdisp/2_nonad_rot_cropped.png}
    \caption{\textcolor{Corr}{Schematic depicting nonadiabatic obliquity
...
When $\theta_{\rm sd, i}\neq 0$, \textcolor{Corr}{we find that the final
obliquity $\theta_{\rm f}$ varies over a range of values for a given
$\theta_{\rm sd, i}$, as can be seen in Fig.~\ref{fig:nonad_3_ensemble}. The
...
\end{equation}
\textcolor{Corr}{
This can be understood as follows (see Fig.~\ref{fig:2nonadrot}). In the
beginning ($\eta = \eta_{\rm i} \gg 1$), the initial spin vector precesses
around $\uv{l}_{\rm d}$ on a cone with opening half-angle $\theta_{\rm sd, i}$
(more precisely, the cone is centered on CS2, which coincides with $\uv{l}_{\rm
d}$ as $\eta_{\rm i} \to \infty$). Note that for $\eta \gg 1$, the adiabaticity
condition is easily satisfied: Eq.~\eqref{eq:w_lib} gives $\omega_{\rm lib}
\simeq \eta$ while Eq.~\eqref{eq:dq2dt} gives $\abs{\rdil{\theta_2}{\tau}} \sim
\epsilon \tan\p{\theta_2 - I} \ll \eta$. As $\eta$ decreases, the system will
transition from being adiabatic to being nonadiabatic, since $\epsilon \gtrsim
\epsilon_{\rm c}$. The evolution of the system can thus be decomposed into
two phases: (i) when the evolution is adiabatic, the spin vector will precess
around the slowly-moving CS2, then (ii) when the evolution becomes nonadiabatic,
the spin vector stops tracking the quickly-evolving CS2. During the adiabatic
evolution of phase (i), the angle between CS2 and the spin vector is
approximately unchanged due to conservation of phase space area}\footnote{
\textcolor{Corr}{This approximation assumes sufficiently small $\theta_{\rm sd,
i}$ such that libration about CS2 remains approximately circular throughout
phase (i) (initially, when $\eta \to \infty$, all librations are circular about
$\uv{l}_{\rm d}$). This assumption breaks down when $\theta_{\rm sd, i}$ is
sufficiently large that librating orbits become non-circular as $\eta$ decreases
before the end of phase (i) (Fig.~\ref{fig:eq_1contours} illustrates that
libration cycles farther from CS2, corresponding to a larger $\theta_{\rm sd,
i}$, are less circular for a given $\eta$).
...
This causes the deviation of the data in Fig.~\ref{fig:nonad_3_ensemble} from
Eq.~\eqref{eq:nonad_q_f_dist} for $\theta_{\rm sd, i} \gtrsim 45^\circ$.
}}.
\textcolor{Corr}{Once the evolution enters phase (ii), the precession axis
quickly (on timescale $\ll 1 / \omega_{\rm lib}$) changes to $\uv{l}$ (as $\eta
\to \eta_{\rm f} \ll 1$). Precession about $\uv{l}$ does not change the
