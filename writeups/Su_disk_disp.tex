...
Obliquities from Planet-Disc Interactions}
...
disk driving orbital precession of an interior planet. We study this scenario
analytically in this paper, and obtain the mapping between the initial spin
orientation and the final obliquity. We show that (i) under adiabatic evolution
(i.e.\ the disk dissipates at a sufficiently slow rate), the final planetary
obliquities bifurcate into distinct tracks governed by interactions with the
resonance; (ii) under nonadiabatic evolution, a broad range of obliquities can
be excited. We obtain analytical expressions for the final obliquities for most
interesting regimes of parameter space. The dynamical system studied in this
paper is an example of ``Colombo's top'', and our analysis and results can be
...
obliquities from planet-disc interaction with a dissipating disc.
...
/ L$, and $\uv{l}_{\rm d} \equiv \bm{L}_{\rm d} / L_{\rm d}$.
...
d}$ [Eq.~\eqref{eq:dmd_dt}] be much larger than all timescales of the dynamical
...
the separatrix. Thus, a \emph{weak adiabaticity criterion} is
that\textcolor{Corr}{, for all equilibria/fixed points, the local
circulation/libration timescales are much shorter than motion of the
equilibrium}. If this criterion is satisfied, then the system will evolve
adiabatically for most of its evolution save one or two separatrix crossings.
...
system. The weak adiabaticity criterion \textcolor{Corr}{is equivalent to
requiring that, for all times other than separatrix crossing, the obliquity of
CS2 ($\theta_2$) evolve over longer timescales than the local libration period
about CS2, or
\begin{equation}
    \rd{\theta_2(t)}{t} \lesssim \frac{\omega_{\rm lib}(t)}{2\pi},\label{eq:ad_base}
\end{equation}
where
\begin{equation}
    \omega_{\rm lib}(t) = \sqrt{\eta(t)\sin I \sin \theta_2(t)
            \p{1 + \eta \sin I \csc^3 \theta_2(t)}},\label{eq:w_lib}
\end{equation}
is the libration frequency about CS2 at time $t$ (Appendix~\ref{ss:eigens}).}
This formula differs from that given in \citet{millholland_disk}, where the
$\csc^3\!\theta_2$ term is neglected and the square root is missing\footnote{The
missing $\csc^3\!\theta_2$ term can be traced to a $\theta \gg I$ approximation
made in Eq.~(3) of \citet{ward2004II}. Since $\theta_2 \sim I$ for $\eta \sim 1$
(Fig.~\ref{fig:cs_locs}), this approximation is not always valid.}.
\textcolor{Corr}{Differentiating Eq.~\eqref{eq:cs_zero} gives
\begin{equation}
    \rd{\theta_2}{t} = \epsilon \frac{\eta \sin\p{\theta_2(t) - I}}{
        \cos\p{2\theta_2(t)} - \eta \cos\p{\theta_2(t) - I}},\label{eq:dq2dt}
\end{equation}
where $\epsilon = -\rdil{(\ln \eta)}{t}$ [Eq.~\eqref{eq:eps_def}]. For a given
$\epsilon$, Eqs.~(\ref{eq:ad_base}--\ref{eq:dq2dt}) will be satisfied for all
times if they are satisfied near $\eta \simeq 1$, where $\dot{\theta}_2 \simeq
\epsilon$. Thus, weak adiabiticity requires
\begin{equation}
    \epsilon \lesssim \epsilon_{\rm c},\label{eq:ad_constr}
\end{equation}
where
\begin{equation}
    \epsilon_{\rm c} \equiv \p{\frac{\omega_{\rm lib}}{t}}_{\eta = 1} =
        \frac{1}{2\pi}\sqrt{\sin I \sin \theta_{\rm 2c}
            \p{1 + \sin I \csc^3 \theta_{\rm 2c}}},
\end{equation}
where $\theta_{\rm 2c}$ is the location of CS2 for $\eta = 1$.
% theta_{2c} = 31^\circ$
} For $I = 5^\circ$, we find $\epsilon_{\rm c} = 0.0431$. Since our criterion is
only a weak condition for adiabaticity, we use $\epsilon = 3 \times 10^{-4}$ in
our ``adiabatic'' calculations below. We explore the consequences of
nonadiabatic evolution in Section~\ref{s:nonad}.
...
$\theta_{\rm sd, i}$ using Eq.~\eqref{eq:thetaf}
is somewhat \textcolor{Corr}{subtle} but yields good agreement with numerical
results.
...
condition is CS2). \textcolor{Corr}{The final obliquity in this case, denoted
...
\begin{figure}
    \centering
    \includegraphics[width=0.6\columnwidth]{plots_diskdisp/2_nonad_rot.png}
    \caption{\textcolor{Corr}{Schematic depicting the calculation of
    $\theta_{\rm f}$ for nonadiabatic evolution when $\theta_{\rm sd, i} > 0$,
    where $\theta_{\rm 0f}$ is given by Eq.~\eqref{eq:theta_2f}. The figure
    shows a projection onto the $x$-$z$ plane. For a given $\theta_{\rm sd, i}$,
    the set of initial conditions forms a ring about CS2 (solid red area) and
    evolves to a ring about $\theta_{\rm 0f}$ (hatched red area) under
    nonadiabatic evolution. The final obliquities are the projection of this
    final cone onto the $\uv{l}$ axis (horizontal green
    lines).}}\label{fig:2nonadrot}
\end{figure}
\textcolor{Corr}{
When $\theta_{\rm sd, i}\neq 0$, the calculation of final obliquities is best
described by Fig.~\ref{fig:2nonadrot}, and yields the range of obliquities
\begin{equation}
    \abs{\theta_{\rm 0f} - \theta_{\rm sd, i}}
        \lesssim \sin\theta_{\rm f}
        \lesssim \theta_{\rm 0f} + \theta_{\rm sd, i}
    .\label{eq:nonad_q_f_dist}
\end{equation}
The absolute value in the lower bound is necessary to capture the behavior of
the projected obliquities in Fig.~\ref{fig:2nonadrot} when the hatched area
encloses $\uv{l}$.}

\textcolor{Corr}{
The process depicted in Fig.~\ref{fig:2nonadrot} has a physically clear
interpretation as a two-step process: (i) when $\eta$ is sufficiently large, the
evolution of the system is always adiabatic [$\dot{\theta}_2 \to 0$,
Eq.~\eqref{eq:dq2dt}], so $\uv{s}$ precesses around CS2 with constant angle
$\theta_{\rm sd, i}$ as $\eta$ decreases; (ii) when $\eta$ has decreased
sufficiently that the system becomes nonadiabatic [violating
Eq.~\eqref{eq:ad_base}], the obliquity of the system at that instant is frozen
in as the final obliquity.}

Fig.~\ref{fig:nonad_3_ensemble} shows the numerical result of $\theta_f$ vs
$\theta_{\rm sd,i}$ for $I =5^\circ$ and $\epsilon = 0.3$. We see that
...
\begin{enumerate}
    \item In the adiabatic limit (i.e.\ the disk dissipates at a
        ...
        of $\theta_{\rm f}$ for nonzero initial spin-disk misalignment angles
        $\theta_{\rm sd, i}$ (see Fig.~\ref{fig:ad_ensemble}). We are able to
        capture both the exact final $\theta_{\rm f}$ values and the
        probabilities of achieving each value via careful accounting of
        adiabatic invariance and separatrix crossing dynamics.
...
We thank Kassandra Anderson for discussion and assistance in the early phase of
